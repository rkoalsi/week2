...
describe('Custom Tests', function () {
  ...
  it('[assignment] iii. Alice deposits 0.13 ETH in L1 -> Alice sends 0.06 ETH to Bob in L2 -> Bob withdraws all his funds in L2 -> Alice withdraws all her remaining funds in L1 -> assert all relevant balances are correct.', async () => {
    const { tornadoPool, token, omniBridge } = await loadFixture(fixture);
    const aliceKeypair = new Keypair(); // contains private and public keys
    ...
    //omniBridge to verify that withdraw by Alice was from L1.
    const { args, extData } = await prepareTransaction({
      tornadoPool,
      outputs: [aliceChangeUtxo],
    });
    ...
    const aliceWithdrawAmount = utils.parseEther(
      (aliceDepositAmount.sub(bobSendAmount) / 1000000000000000000).toString()
    );
    const recipient = '0x38d918D2Cf6D71Af1038183647a8B636068AE4Fd';
    ...
    //Recipient balance should be 0 and omniBridge balance should be equal to the remaining amount withdrawn by Alice since transaction takes place in L1
    const recipientBalance = await token.balanceOf(recipient);
    console.log('recipientBalance =', recipientBalance / 1000000000000000000);
    expect(recipientBalance).to.be.equal(0);
    const omniBridgeBalance = await token.balanceOf(omniBridge.address);
    console.log('omniBridgeBalance =', omniBridgeBalance / 1000000000000000000);
